    ...
    <div class="grid lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div class="flex gap-4 p-5 border-b border-gray-50 last:border-0" wire:key="cart-item-{{ $item->id }}">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-3 mt-3">
                            <div class="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                                <button type="button" wire:click="decrement({{ $item->id }})" wire:loading.attr="disabled" class="px-2.5 py-1.5 hover:bg-gray-50 text-gray-500 text-sm">−</button>
                                <span class="w-10 text-center border-x border-gray-200 py-1.5 text-sm inline-block">{{ $item->quantity }}</span>
                                <button type="button" wire:click="increment({{ $item->id }})" wire:loading.attr="disabled" class="px-2.5 py-1.5 hover:bg-gray-50 text-gray-500 text-sm">+</button>
                            </div>

                            <button type="button" wire:click="remove({{ $item->id }})" wire:loading.attr="disabled" wire:target="remove({{ $item->id }})" class="text-xs text-red-400 hover:text-red-600 transition-colors">
                                Remove
                            </button>
                        </div>
                    </div>
